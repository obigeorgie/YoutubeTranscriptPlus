...
import { BRAND_DOMAIN, BRAND_FULL_NAME, BRAND_SHORT_NAME } from '@/lib/brand';
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-6xl flex-col px-4 py-8 sm:px-6 lg:px-8">
      <header className="mb-10 flex flex-col gap-6 rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-sm uppercase tracking-[0.3em] text-sky-300/80">{BRAND_SHORT_NAME} · {BRAND_DOMAIN}</p>
          <h1 className="mt-2 text-3xl font-bold tracking-tight sm:text-5xl">{BRAND_FULL_NAME}</h1>
          <p className="mt-3 max-w-2xl text-sm text-white/70 sm:text-base">
            Paste a public YouTube URL, fetch the transcript with {BRAND_SHORT_NAME}, and generate a quick extractive summary.
          </p>
        </div>
        <AuthButton />
      </header>
